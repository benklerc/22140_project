...
dxdResult_08_df <- readRDS("../data/dexseq_results_08_age.rds")
dxdResult_77_df <- readRDS("../data/dexseq_results_77_age_sub.rds")
...
deseq_08_df <- as.data.frame(readRDS("../data/deseq2_results_08_age.rds"))
deseq_77_df <- as.data.frame(readRDS("../data/deseq2_results_77_age_sub.rds"))
...
alpha <- 0.05
dex_gene_08 <- dexseq_to_genelevel(dxdResult_08_df)
...
```{r}
c(sum(deseq_gene_08$deseq_sig_08), sum(dex_gene_08$dex_sig_08), sum(both_08$dex_sig_08), sum(both_08$deseq_sig_08))
```
...
dex_gene_77 <- dexseq_to_genelevel(dxdResult_77_df)
...
```{r}
c(sum(both_77$dex_sig_77), sum(both_77$deseq_sig_77))
```
...
## fgseaMultilevel for DESeq2 and DEXSeq 77
...
nes_comparison_plot_77 <- ggplot(cmp_77, aes(x = NES_de_77, y = NES_dx_77, color = sig_cat_77)) +
  ...
  labs(
    ...
    y = "NES (DEXSeq fgsea)",
    color = "Significance",
    title = "NES comparison: DESeq2 vs DEXSeq"
  ) +
  theme_minimal()
print(nes_comparison_plot_08)
print(nes_comparison_plot_77)
...
```{r}
dxdResult_77_age_subdf <- as.data.frame(readRDS("../data/dexseq_results_77_age_sub.rds"))
dxdResult_77_gender_df <- as.data.frame(readRDS("../data/dexseq_results_77_sub.rds"))
dxdResult_08_gender_df <- as.data.frame(readRDS("../data/dexseq_results_08.rds"))
dxdResult_08__gender_age_df <- as.data.frame(readRDS("../data/dexseq_results_08_age.rds"))
```

```{r}
count_sig_genes_dexseq <- function(dex_df, alpha = 0.05) {
  
  dex_gene <- dex_df %>%
    as_tibble() %>%
    filter(!is.na(stat)) %>%                        # remove invalid rows
    dplyr::select(
      gene_id   = groupID,
      isoform_id = featureID,
      pvalue,
  ...
  sum(dex_gene$padj < alpha, na.rm = TRUE)
}
...
```{r}
print(paste("dex77 with age and gender", count_sig_genes_dexseq(dxdResult_77_age_subdf)))
```

```{r}
(paste("dex77 with gender", count_sig_genes_dexseq(dxdResult_77_gender_df)))
```

```{r}
(paste("dex88 with age and gender", count_sig_genes_dexseq(dxdResult_08__gender_age_df)))
```

```{r}
(paste("dex88 with gender", count_sig_genes_dexseq(dxdResult_08_gender_df)))
```
