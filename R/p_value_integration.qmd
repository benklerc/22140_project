...
full_diff_analysis <- full_diff_analysis |>
  rowwise() |>
  mutate(
  joint_pval_fisher = metap::sump(c( joint_pval_fisher_08, joint_pval_fisher_77))$p  ) |>
  ungroup()
```
...
```{r}
int_diff_analysis_77_joint <- int_diff_analysis_77 %>% dplyr::select(Genes, joint_pval_fisher_77)
int_diff_analysis_08_joint <- int_diff_analysis_08 %>% dplyr::select(Genes, joint_pval_fisher_08)
