...
dxdResult_77 <- readRDS('../data/dexseq_results_77_sub.rds')
```
...
# Gender Distribution by Group (Bar Plot)
plot_77_gender_by_group <- metadata_df_77 |>
  ggplot(aes(x = gender, fill = groups)) +
  geom_bar(position = "dodge") +
  labs(
...
# Age Distribution by Group (Density Plot)
plot_77_age_by_group <- metadata_df_77 |>
  ggplot(aes(x = age, fill = groups)) +
  geom_density(alpha = 0.5) + # alpha=0.5 allows overlapping curves to be visible
  labs(
  ...
  # === FIX: Manually set x-axis limits for padding ===
  scale_x_continuous(limits = c(min(metadata_df_77$age, na.rm = TRUE) - 10, 
                                max(metadata_df_77$age, na.rm = TRUE) + 10))
...
-   Do we have DeSEQ2 GSEA with the subsetted data?
