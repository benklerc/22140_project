...
integrated_08 <- readRDS("../data/integrated_08.rds")
```
...
#getting top 200 integrated genes
sig_iso_integrated <- integrated_08 %>%
  filter(!is.na(adj_joint_pval) & adj_joint_pval < alpha) %>%
  filter(Genes %in% ppi_genes)

top_integrated_08 <- integrated_08 %>%
  arrange(adj_joint_pval) %>%
  distinct(Genes, .keep_all = TRUE) %>%
  slice_head(n = 200) %>%
  pull(Genes)
# Define combined seeds
seeds <- top_integrated_08
...
print(paste("Total unique seeds:", length(top_integrated_08)))
```

**This means that the Dexseq and Deseq2 only share 1 of their top 100 significant genes???**
