...
# Function to filter rows where no sample exceeds threshold
filter_low_expression <- function(count_matrix, threshold = 5) {
  # Check if any sample in each row has a value > threshold
  keep_rows <- apply(count_matrix, 1, function(row) any(row > threshold))
  
  # Return filtered matrix
  return(count_matrix[keep_rows, ])
}
...
isoform_count_08 <- filter_low_expression(isoform_count_08, threshold = 5)
isoform_count_77 <- filter_low_expression(isoform_count_77, threshold = 5)

# Subset isoInfo to match the transcripts present in the count matrix
## We should also do this subset info matching for count_77
isoInfo_sub <- isoInfo[match(rownames(isoform_count_77), isoInfo$transcript_id), ]
...
saveRDS(dxdResult, file = "../data/dexseq_results_77_full.rds")
saveRDS(dxdSubset, file = "../data/dexseq_dataset_object_77_full.rds")
