...
metadata_df_77 <- metadata_df_77 |>
  ...
  filter(age > 45) # We only filter age for 77 dataset
...
dxdSubset <- testForDEU(
  dxdSubset,
  fullModel = ~ sample + exon + groups:exon + gender:exon + age:exon,
  reducedModel = ~ sample + exon + gender:exon + age:exon
)
...
saveRDS(dxdResult, file = "../data/dexseq_results_08_age.rds")
saveRDS(dxdSubset, file = "../data/dexseq_object_08_age.rds")
```
...
#| message: false
#| warning: false
rownames(metadata_df_77) <- metadata_df_77$sample_id
isoform_count_77 <- isoform_count_77[, rownames(metadata_df_77), drop = FALSE]
...
```{r}
saveRDS(dxdResult_77, file = "../data/dexseq_results_77_age_sub.rds")
saveRDS(dxdSubset_77, file = "../data/dexseq_object_77_age_sub.rds")
```
